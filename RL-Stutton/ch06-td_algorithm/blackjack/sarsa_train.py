...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        while not game_ended:
            ...
            ap = policy(pi, epsilon / epoch, sp)
            N[(s, a)] += 1
            update_delta = (r + q[sp[0], sp[1], sp[2], ap] - q[s[0], s[1], s[2], a]) / N[(s, a)]
    ...
    plt.xlabel('Episodes')
    plt.ylabel('Win Rate')
    fig.savefig('sarsa_win_rate.png')
    ...
    plt.pcolor(pi[:, :, 0])
    plt.colorbar()
    fig.savefig('sarsa_pi_no_ace.png')
    ...
    plt.pcolor(pi[:, :, 1])
    plt.colorbar()
    fig.savefig('sarsa_pi_with_ace.png')
